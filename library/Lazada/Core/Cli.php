<?php
/**
 * File containing the Evd_Core_Cli class.
 *
...
class Lazada_Core_Cli
{
    ...
    static function parseOptionString( $configString, &$optionConfig )
    {
        ...
        while ( $i < $len )
        {
            ...
            if ( $option == '[' )
            {
                ...
                if ( $end === false )
                {
                    Evd_Core_Log::writeLog("Missing end marker ] in option string at position $i", __METHOD__ );
                ...
                {
                    Evd_Core_Log::writeLog( "Start marker [ found in option string at position, it should not be present. Skipping current option" . ( $i + 1 + $startMarkerPos ),
                                         'Evd_Core_Cli::parseOptionString' );
                    continue;
                }
    ...
    */
    function getOptions( $config, $argumentConfig, $arguments = false )
    {
        ...
        if ( is_string( $config ) )
            $config = Evd_Core_Cli::parseOptionString( $config, $tmpConfig );
        if ( is_string( $argumentConfig ) )
            $argumentConfig = Evd_Core_Cli::parseOptionString( $argumentConfig, $tmpArgumentConfig );
    ...
    /**
     * Returns a shared instance of the Evd_Core_Cli class.
     *
     * @return Evd_Core_Cli
     */
    static function instance()
    {
        if ( !isset( $GLOBALS['CLIInstance'] ) ||
             !( $GLOBALS['CLIInstance'] instanceof Evd_Core_Cli ) )
        {
            $GLOBALS['CLIInstance'] = new Evd_Core_Cli();
        }

        return $GLOBALS['CLIInstance'];
    }

    /*!
     \return \c true if instance of Evd_Core_Cli already exists otherwise \c false.
    */
    static function hasInstance()
    {
        return isset( $GLOBALS['CLIInstance'] ) && $GLOBALS['CLIInstance'] instanceof Evd_Core_Cli;
    }
}
